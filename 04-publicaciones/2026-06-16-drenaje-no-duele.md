# Post — El drenaje linfático NO duele (FAQ 1)

- **Fecha:** Martes 16/06/2026 · 1:00 pm
- **Pilar:** Educación / Autoridad
- **Formato:** Carrusel educativo (5 slides)
- **Estado:** 🟡 Listo para publicar

## Objetivo
Posicionar para "masajes postoperatorios Maracaibo" y desmontar el mito "si no duele,
no funciona". Contenido para guardar y compartir (alto alcance).

## Visual (lienzo 4:5 · 1080x1350)
- **Slide 1 (portada):** fondo turquesa #0FA98F. Título "¿El drenaje TIENE que doler para
  funcionar?" (Playfair) + "Te lo explico después de 30 años 👉" (Montserrat). Logo.
- **Slide 2 (mito):** fondo blanco. "MITO ❌" + "Si no duele, no está funcionando."
- **Slide 3 (realidad):** fondo azul #2B2E91, texto blanco. "REALIDAD ✅" + "Un verdadero
  drenaje linfático es suave, rítmico… y alivia."
- **Slide 4 (porqué):** fondo blanco. Título: "Entonces, ¿qué hace realmente?" +
  cuerpo: "Ayuda a tu cuerpo a movilizar los líquidos retenidos tras la cirugía,
  aliviando la presión y la hinchazón casi de inmediato."
- **Slide 5 (cierre + CTA):** foto de la Lic. "Tu recuperación merece manos con experiencia."
  + "Escríbeme CITA por DM 💬" + logo + 📍 Clínica Empire.

## Caption
```
Masajes postoperatorios en Maracaibo: el dolor NO es señal de que funcione 🤍

Una de las frases que más escucho en consulta es:
"Licenciada, póngame duro que así sirve más".

Y aquí va la verdad después de 30 años: eso es un mito.

Un verdadero drenaje linfático manual es suave, rítmico y delicado. 
No te lastima: te alivia. Porque su función es ayudar a tu cuerpo a 
eliminar los líquidos retenidos tras la cirugía, y eso baja la presión 
y la inflamación casi de inmediato.

Si un masaje "post-operatorio" te deja morados o llorando de dolor, 
no es técnica… es falta de ella.

Tu recuperación merece manos con experiencia.
Escríbeme CITA por DM y te explico tu proceso 💬

📍 Clínica Empire, Av. Fuerzas Armadas, Maracaibo

#PostOperatorioMaracaibo #DrenajeLinfaticoMaracaibo #MasajesPostQuirurgicos #EsteticaMaracaibo #CosmiatraMaracaibo
```

## Detalles técnicos
- **Geotag:** Clínica Empire
- **Alt Text:** "drenaje linfático post operatorio en Maracaibo"

## Stories de apoyo
1. Encuesta: "¿Creías que el drenaje tenía que doler? Sí / No 👀"
2. Reposteo del carrusel: "Guárdalo para tu recuperación 📌"

## Métricas (llenar después)
- Alcance:
- Likes:
- Guardados:
- Comentarios:
- DMs / "CITA" recibidos:
- Aprendizaje:
